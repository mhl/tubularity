...
namespace itk
{
																					...
																					float radius, float sigma0)
	{
		...
		while(!it.IsAtEnd())
		{
			...
			if(normU >= eps)
			{
				...
				{
					itkGenericExceptionMacro("Oriented Flux filter in the Fourier imlemented only for dimensions 2 and 3");
				}
			}
			it.Set(value);
			++it;
	...
	template <typename TInputImage, typename TOutputImage>
	typename FFTOrientedFluxMatrixImageFilter< TInputImage, TOutputImage >::InputSizeType
	FFTOrientedFluxMatrixImageFilter< TInputImage, TOutputImage >
