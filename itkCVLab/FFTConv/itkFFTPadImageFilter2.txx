/*=========================================================================
  ...
  Module:    $RCSfile: itkFFTPadImageFilter2.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkFFTPadImageFilter2_txx
#define __itkFFTPadImageFilter2_txx
...
FFTPadImageFilter2<TInputImage, TInputKernel, TOutputImage, TKernelOutput>
::FFTPadImageFilter2()
{
  ...
  if( m_PadMethod == NO_PADDING )
    ...
    for( int i=0; i<ImageDimension; i++ )
      ...
      if( m_GreatestPrimeFactor > 1 )
        {
					 /** The only differece with Gaetan Lehmann inplementation is here:
					  *  An additional condition has been added so that the size of 
					  *  the padded image is even. We added such a condition to get 
					  *  rid of a half pixel drift that happen when fftw is used.
					  *  Such a drift is appearante when the size is not odd
					  *  and it is visible only on the right hand side of the image.
					  *
					  *  \modified by: F. Benmansour
					  */
        while( greatestPrimeFactor( region0.GetSize()[i] + s1 ) > m_GreatestPrimeFactor  && ((region0.GetSize()[i] + s1)%2))
          {
          s1++;
          }
        }
