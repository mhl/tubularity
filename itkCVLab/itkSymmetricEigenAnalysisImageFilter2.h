...
namespace itk {
	 ...
	 *        of an input image. Input pixel type represent a  symmetric 
	 *		  matrix and must provide write access for the [][] operator.
	 *
	 *		  TODO
	 *
...
//		typedef typename EigenValuePixelType::ValueType				EigenValueElemValueType;
		typedef typename EigenVectorImageType::PixelType			EigenVectorPixelType;
//		typedef typename EigenVectorPixelType::ValueType			EigenVectorElemValueType;
		typedef typename FirstEigenVectorOrientImageType::PixelType	EigenVectorOrientPixelType;
//		typedef typename EigenVectorOrientPixelType::ValueType		EigenVectorOrientElemValueType;
		 ...
		 * as the NxN eigenvector matrix, where N is the size of the input matrix. By default, 
		 * the filter generates the eigenvector image. */
		...
		/** Methods to turn on/off generation of the first eigenvector orientation image 
		 * with the pixels set as the first N-1 elements of the first eigenvector (i.e., 
		 * the one that corresponds to the first eigenvalue in the ordered eigenvalue list), 
		 * where N is the size of the input matrix. Note that, the last element can be 
		 * easily recovered from the first N-1 ones since the eigenvectors are always 
		 * of unit magnitude. By default, the filter does not generate this image. */
...
//#endif
	...
	protected:
		SymmetricEigenAnalysisImageFilter2(); // TODO USe ProcessObject::SetNumberOfOutputs(1) and ProcessObject::SetNumberOfRequiredOutputs(1)
